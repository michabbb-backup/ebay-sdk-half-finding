...
assert(file_exists(__DIR__ . '/artifacts/dts-half-finding.zip'));
assert(file_exists(__DIR__ . '/artifacts/dts-half-finding.phar'));
...
$client->post(
    $uploadUrl . '/assets?name=dts-half-finding.zip',
    array('Content-Type' => 'application/zip'),
    fopen(__DIR__ . '/artifacts/dts-half-finding.zip', 'r')
)->send();

$client->post(
    $uploadUrl . '/assets?name=dts-half-finding.phar',
    array('Content-Type' => 'application/phar'),
    fopen(__DIR__ . '/artifacts/dts-half-finding.phar', 'r')
)->send();
